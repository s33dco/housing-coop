...
describe 'a person' do 
  ...
  it "rejects an improperly formatted first name" do
    ...
    names.each do |name|
      person = Person.new(firstname: name)
      person.valid?
  ...
  it "rejects an improperly formatted last name" do
    ...
    names.each do |name|
      person = Person.new(lastname: name)
      person.valid?
  ...
  it "requires a unique, case insensitive email address" do
    ...
    person2 = Person.new(email: person1.email.upcase)
    person2.property = property
    
    person2.valid?
    
    expect(person2.errors[:email].first).to eq("Email address already used")
  end

  it "rejects incorrect phone numbers" do
    phones = %w[1 0121 123456 004401234567898 01278678678ext123 ]
    phones.each do |phone|
      person = Person.new(phone: phone)
      person.valid?
