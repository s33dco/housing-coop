...
RSpec.describe 'Rent Balance' do
		...
		it 'identifies when a property is vacant by date combinations set(2)' do
			property = build_stubbed(:property, rent_period_start:4.weeks.ago, first_day_of_next_rent_period:2.weeks.ago, moving_out_date:3.weeks.ago)
			expect(property.vacant?).to eq(true)
			expect(property.standard_process?).to eq(false)
			expect(property.rent_change_period?).to eq(false)
		end 
		...
		it 'identifies when rent changes after moving_out_date'do
