...
describe 'a calendar event' do 
	  ...
	  it "requires a date and time" do
	    event = Calendar.new(when: "")
	    event.valid?
	    expect(event.errors[:when].any?).to eq(true)
	  end

	  it "requires a title" do
	    event = Calendar.new(title: "")
	    event.valid?
  	...
  	names = [ "!ks<>", " ", "1-2-2!!!"]
  	names.each do |name|
		    event = Calendar.new(details: name)
		    event.valid?
	    	expect(event.errors[:details].any?).to eq(true)
	    end
  	end
